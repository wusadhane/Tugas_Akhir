...
# Fungsi untuk menampilkan grafik Efficient Frontier
def show_efficient_frontier():
    # Ambil semua data dari tabel
    rows = main_table.get_children()
    if not rows:
        messagebox.showerror("Ups!", "Tidak ada data untuk ditampilkan.")
        return
    ...
    for row in rows:
        values = main_table.item(row, "values")
        portfolio_data.append((float(values[3]), float(values[2])))  # (Risk, Expected Return)

    # Plot Efficient Frontier
    plt.figure(figsize=(10, 6))
    plt.scatter([p[0] for p in portfolio_data], [p[1] for p in portfolio_data], c='blue')
...
# ---------------------------------BUTTON_SHOW_FRONTIER------------------------------

# Tombol untuk menampilkan kurva Efficient Frontier
frontier_button = Button(text="Show Efficient Frontier", command=show_efficient_frontier)
frontier_button.place(x=250, y=400)
